...
_COLLEGE_CAREERS_ORIG = dict(CollegeCareers)
_CAREERS_ORIG = dict(Careers)
...
def yesmatch(query):
    return bool(re.match(r'Yes|yes|y|Yes\.|Y', query))

def brief_pause(secs=0.5):
    time.sleep(secs)
...
while True:
    try:
        NumPlayers = int(input("How many people are playing today? "))
        if NumPlayers < 1:
...
while count < NumPlayers + 1:
    ...
    if yesmatch(PlayerCollege):
        if not CollegeCareers:
            CollegeCareers.update(_COLLEGE_CAREERS_ORIG)
        CareerSet = CollegeCareers
        Players[PlayerSlot].update({'College': 'Yes'})
    else:
        if not Careers:
            Careers.update(_CAREERS_ORIG)
        CareerSet = Careers
    ...
    if yesmatch(PlayerKidChoice):
        kids = random.randint(0, 6)
    else:
        kids = random.choices([0, 1, 2], weights=[70, 20, 10])[0]
...
print('')
brief_pause()
print('Thanks for playing the Python Game of Life!\n')
print(Players)
